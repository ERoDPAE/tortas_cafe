...
setwd("//arba-30m/GRP/IT/Shared/XFCR03/06_Datos_torta_anidada/export/")
...
df_list <- list(export_1.csv, export_2.csv, export_3.csv,export_4.csv,
                ...
                export_11.csv,export_12.csv,export_13.csv,export_14.csv)
...
library(readxl)
df_cluster_resultados_finales <- read_excel("//arba-30m/GRP/IT/Shared/XFCR03/06_Datos_torta_anidada/export/df_cluster_resultados_finales.xlsx")
...
tortas_q_cafes <- function(base_agrupada,Titulo){
  ...
  if(sum(interior$Cantidad)<3500){
    ...
    torta_ext=floating.pie(0.5,0.5,exterior$porcentaje_abs,radius=0.15,border="white",col=unlist(colores_ext))
    torta_int=floating.pie(0.5,0.5,interior$porcentaje_abs,radius=0.10,border="white",col=unlist(col_int))
    mtext(Titulo,side=3,adj=0,cex=1.5)
    #showRatioThreshold = F
    pie.labels(x=0.5, y=0.5, torta_int, paste0(interior$Nivel1,"\n", round(interior$porcentaje_abs*100),"%"),minangle=0.2,radius=0.03,cex=0.65,font=2,col="black")
...
```{r torta_gral, echo=FALSE,warning=FALSE,message=FALSE}
tortas(base_agrupada,"Total Red")
tortas_normalizadas(base_agrupada,"Normalizada")
tortas_q_cafes(base_agrupada,"Por uds bebidas en tkt")
tortas_cafes_local(base_agrupada,"En Local")
distribucion_horaria(base_agrupada)
distribucion_horaria2(base_agrupada)

```

## Nivel Cluster

```{r torta_por_cluster, echo=FALSE,warning=FALSE,message=FALSE}
for (i in sort(unique(na.omit(base_agrupada$Cluster_combinado)))){
  temp <- base_agrupada[base_agrupada$Cluster_combinado==i,]
  tortas(temp,paste0("Cluster - ",i))
  tortas_normalizadas(temp,paste0("Norm. - ",i))
  tortas_q_cafes(temp,paste0("Por uds - ",i))
  tortas_cafes_local(temp,"En Local")
  distribucion_horaria(temp)
  distribucion_horaria2(temp)
...
```{r torta_por_estación, echo=FALSE,warning=FALSE,message=FALSE}
for (i in sort(unique(na.omit(base_agrupada$estacion)))){
  temp <- base_agrupada[base_agrupada$estacion==i,]
  cluster <- max(temp$Cluster_combinado)
  tortas(temp,paste0(i," ",cluster))
  tortas_q_cafes(temp,paste0("Por uds - ",i))
  tortas_cafes_local(temp,"En Local")
  distribucion_horaria(temp)
  distribucion_horaria2(temp)
