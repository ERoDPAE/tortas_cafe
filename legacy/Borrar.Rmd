...
df_list <- list(export_1.csv, export_2.csv, export_3.csv,export_4.csv,
                export_5.csv,export_6.csv,export_7.csv,export_8.csv)
...
df_cluster_resultados_finales <- df_cluster_resultados_finales %>% dplyr::select(`PBL Name`,Cluster_combinado) %>% mutate(`PBL Name`=toupper(`PBL Name`) )
...
pie.labels(x=0.5, y=0.5, torta_ext_2, paste0(exterior_2$Nivel3,"\n", round(exterior_2$mix_seg*100)," %"),minangle=0.20,radius=0.16,cex=0.65,font=2,col="black")
pie.labels(x=0.5, y=0.5, torta_ext_3, ifelse(exterior_3$Nivel4=="NA","",paste0(exterior_3$Nivel4,"\n", round(exterior_3$mix_seg*100)," %")),minangle=0,radius=0.21,cex=0.65,font=2,col="black")
pie.labels(x=0.5, y=0.5, torta_ext_4, ifelse(exterior_4$Nivel5=="Sandwich",paste0(exterior_4$Nivel5,"\n", round(exterior_4$mix_seg*100)," %"),""),minangle=0,radius=0.30,cex=0.65,font=2,col="black")
...
for (i in sort(unique(na.omit(base_agrupada$Cluster_combinado)))){
  ...
  g1<-tortas(temp,paste0("Cluster - ",i))
  g2<-distribucion_horaria(temp)
  g3<-tortas_normalizadas(temp,paste0("Cluster - ",i))
...
for (i in sort(unique(na.omit(base_agrupada$estacion)))){
  tortas(base_agrupada[base_agrupada$estacion==i,],i)
  distribucion_horaria(base_agrupada[base_agrupada$estacion==i,])
}
```
